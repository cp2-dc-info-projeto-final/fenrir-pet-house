    ...
    <div class="fenrir-login">
        <form action="deletauser2.php" method="POST" class="form-container">
          ...
          $sql = "SELECT * FROM func where isadmin = 0;";
                    ...
                    for($i=0; $i < $linhas; $i++){
                        $func = mysqli_fetch_array($res);
                        echo "<br><strong>Nome:</strong> ".$func["nome"]."<br>";
            ...
            if(password_verify($senha, $row['senha'])){
                //deleta conta
                $sql2 ="DELETE FROM func WHERE cod_func = '{$_SESSION["cod_func"]}' ";
                    if(mysqli_query($mysqli, $sql2)){
                        include "envia_email.php";
                        envia_email( $func["email"], "Sentiremos sua Falta!", "Um administrador excluiu sua conta do site da hotelaria canina Fenrir Pet House. Esperamos que você e seu au-migo tenha tido a melhor experiência.");
                        header('location: admfunc.html');
                    }
                }
